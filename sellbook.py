...
def bookRegister():
    ...
    insertBooks = "insert into "+bookTable+" values('"+bid+"','"+bname+"','"+price+"')"
    try:
        cur.execute(insertBooks)
        con.commit()
        messagebox.showinfo('Success',"Book added successfully")
        file = open('records.txt','a') 
        file.write("\n Book with BID = "+bid+"and title ="+bname+" added in inventory ") 
        file.close()
    except:
...
def sellbook(): 
    ...
    headingLabel = Label(headingFrame1, text="Buy Books", bg='black', fg='white', font=('Courier',15))
